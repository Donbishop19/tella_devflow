...
export async function toggleSaveQuestion(params: CollectionBasedParams): Promise<ActionResponse<{saved: boolean}>> {
...
export async function hasSaveQuestion(params: CollectionBasedParams): Promise<ActionResponse<{saved: boolean}>> {
  const validationResult = await action({
    params,
    schema: CollectionBaseSchema,
    authorize: true,
